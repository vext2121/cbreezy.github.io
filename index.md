---
layout: default
---

# Cyber Concepts
#### Using practical scenarios to apply cybersecurity practices!

*   [IT Auditing Demo](./Projects/Demos/audit.md)
*   [Autopsy Digital Forensics Demo](./Projects/Demos/forensics.md)
*   Nmap Network Enumeration Demo (In Progress)


# Security Applications
#### Creating cybersecurity solutions for real networks like your own!

*   Microsoft Sentinel At-Home Through VMWare (In Progress)
*   Plugging in MISP to Microsoft Sentinel through VMWare (In Progress)


# Python for Cybersecurity
#### Using Python to illustrate cybersecurity goals!

*   Firewalls (In Progress)
*   Scanners (In Progress)
